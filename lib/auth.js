...
function Auth() {
	...
	this.init = function (onReady) {
		...
		this.passport.use(Adminion.db.Accounts.createStrategy());
		// serializeUser() and deserializeUser() return the functions passport will use
		this.passport.serializeUser(Adminion.db.Accounts.serializeUser());
		this.passport.deserializeUser(Adminion.db.Accounts.deserializeUser());
...
module.exports = function (adminion, onReady) {
	...
	Adminion.auth = new Auth();

	Adminion.auth.init(onReady);
