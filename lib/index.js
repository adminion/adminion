...
module.exports = function AdminionServer () {
    ...
    function cacheInit () {
        debug.marker('initializing cache...', 'lib/index', 45);

        Server.cache = Cache(Server);

        // when cache module is ready
        Server.cache.on('ready', function () {
            debug.marker('cache initialized', 'lib/index', 51);

            Server.emit('cacheReady');
        });
    }
    ...
    // when db is ready, initialize cache
    Server.on('dbReady', cacheInit);

    // when cache is ready, initialize http
    Server.on('cacheReady', httpInit);
