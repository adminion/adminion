...
module.exports = function (tools) {
    ...
    app.param('email', function (request, response, next, email) {
        tools.db.getAccountByEmail(email, function (err, account) {
            if (err) {
    ...
    function errorHandler (err, request, response) {
        console.trace(err);
        debug.val('err', err, 'lib/http.js', 142);
...
//          debug.emit('var' , 'request.session', request.session, 'lib/http.js', 140);
    ...
    app.get('/accounts/:email/update', verify, function (request, response) {
        ...
        debug.val( 'request.account', request.account, 'lib/http.js', 263);
    ...
    app.get('/games', verify, function (request, response) {
        ...
        tools.db.getGames(count, offset, function (err, games) {
            if (err) {
                errorHandler(err);
            }

            debug.val('games', games, 'lib/http', 359);
            ...
            for (var i=0; i <numGames; i+=1) {
                (function (accountID, gameID) {

                    debug.val('accountID', accountID, 'lib/http', 369);
                    debug.val('gameID', gameID, 'lib/http', 370);

                    tools.db.getAccount(accountID, function (err, account) {
                        if (err) {
                            errorHandler(err);
                        }

                        playerOnes[gameID] = account;
                        numAccounts +=1;
                        
                        if (numAccounts === numGames) {
                        ...
                        debug.val('games', games, 'lib/http', 390);
                        debug.val('playerOnes', playerOnes, 'lib/http', 391);

                    });
                    
                }(games[i].playerOne.accountID, games[i]['_id']));
    ...
    app.get('/games/create', verify, function (request, response) {
        ...
        tools.db.createGame(newGame, function (err, game) {
            if (err) { 
                ...
                debug.val('newGame', game, 'lib/http.js', 425);

                http.emit('newGame', game);
    ...
    app.get('/games/:gameID', verify, function (request, response) {
        ...
        tools.db.getAccount(request.game.playerOne.accountID, function (err, playerOne) {
            debug.val('playerOne', playerOne, 'lib/http', 436);
    ...
    app.get('/games/:gameID/lobby', verify, function (request, response) {      
        ...
        tools.db.getAccount(request.game.playerOne.accountID, function (err, playerOne) {
            debug.val('playerOne', playerOne, 'lib/http', 453);
    ...
    app.get('/games/:gameID/play', verify, function (request, response) {
        ...
        tools.db.getAccount(request.game.playerOne.accountID, function (err, playerOne) {
            debug.val('playerOne', playerOne, 'lib/http', 470);
