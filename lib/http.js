...
function HTTP() {
	...
	//			debug.emit('var' , 'request.session', request.session, 'lib/http.js', 140);
		...
		app.get('/games', Adminion.auth.verify, function (request, response) {
			...
			Adminion.db.Games.where('status').equals('lobby').limit(20).exec(function (err, games) {
				if (err) { 
					console.trace(err)
		...
		app.get('/games/:gameID', Adminion.auth.verify, function (request, response) {
			...
			Adminion.db.Games.findById(request.params.gameID, function (err, game) {
				if (err) { 
					console.trace(err)
					...
					Adminion.db.Accounts.findById(game.playerOne.accountID, function (err, player) {
						if (err) { 
							console.trace(err)
							debug.val('err', err, 'lib/http.js', 361);
							response.render('errors/500', {
