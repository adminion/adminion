...
function Realtime() {
	...
	this.init = function (onReady) {
		...
		// load all the games in 'lobby' or 'play' status into memory
		Adminion.db.Game.$where("status === 'lobby' || status === 'play'")
			.exec(function (err, games) {
				for (var game in games) {
					var gameID = games[game]['_id'];
					self.games[gameID] = games[game];
				}
		...
		io.sockets.on('connection', function (socket) {
			debug.msg('------------------------------ socket connect ----------------------------------', 'lib/realtime', 36);
			debug.val( 'self.sockets', self.sockets, 'lib/realtime', 37);
			...
			if (!this.players[playerID]) {
				...
				this.players[playerID].sockets[socket.id] = socket;
			}
			...
			socket.on('enterLobby', function (gameID) {
				...
				if (gameID === undefined) {
					debug.val('gameID', gameID, 'lib/realtime.js', 53);
				...
				debug.val('playerNum', playerNum, 'lib/realtime.js', 78);

				// if player was allowed to enter
				if (playerNum > -1) {
					debug.msg(handle + ' was allowed to enter the game.', 'lib/realtime', 82);
					...
					debug.val('self.players[playerID]', self.players[playerID], 'lib/adminion', 116);
					debug.val('this.games[gameID].players',this.games[gameID].players, 'lib/realtime.js', 117);
			...
			socket.on('disconnect', function () {
				debug.msg('---------------------------- socket disconnect ---------------------------------', 'lib/realtime', 167);

				debug.val('socket.handshake', socket.handshake, 'lib/realtime.js', 169);
				debug.val('self.sockets', self.sockets, 'lib/realtime.js', 170);
				...
				debug.val('gameID', gameID, 'lib/realtime.js', 183);			
				...
				if (!!gameID) {
					...
					if (self.players[playerID] && self.players[playerID][socket.id] ) {
						...
						if (Object.keys(self.players[playerID]).length === 0) {
							...
							debug.val('game', game, 'lib/realtime.js', 231);
						}

						msg = util.format('%s left game %s', handle, gameID);
						debug.msg(msg, 'lib/realtime.js', 237);
					}						

					debug.val('self.sockets', self.sockets, 'lib/realtime.js', 240);
