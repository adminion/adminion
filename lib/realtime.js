		...
		io.sockets.on('connection', function (socket) {
			...
			socket.on('joinGame', function () {
				...
				if ( game.registration ) {
					...
					if (game.openSeats === 1 && !game.accountOneRegistered && !game.isPlayerOne(accountID)) {
			...
			socket.on('disconnect', function () {
				...
				debug.val('Sockets', Sockets, 'lib/realtime.js', 171);
				...
				if (!!gameID) {
					...
					if (Sockets.byGame(gameID).byPlayer(accountID).length === 0) {
...
module.exports = function (adminion, onReady) {
	...
	Adminion.realtime.on('logon', function (account) {
		Adminion.realtime.accounts[account['_id']] = account;

		debug.val('Adminion.realtime.accounts', Adminion.realtime.accounts, 'lib/realtime.js', 224);
	});

	Adminion.realtime.on('logoff', function (account) {
		delete Adminion.realtime.accounts[account['_id']];
		debug.val('Adminion.realtime.accounts', Adminion.realtime.accounts, 'lib/realtime.js', 229);
	});
