...
function realtime (tools) {
    ...
    function roster (players) {
        ...
        for ( i in players) {
            ...
            account = (player) ? tools.data.getAccount(player.account) : undefined;
    ...
    function initSocket (socket) {
         ...
         */
        socket.on('gameJoin', function onGameJoin () {
            ...
            game = tools.data.getGame(gameID);
            ...
            if (game.isPlayerOne(accountID)) {
                ...
                socket.on('gameKill', function onGameKill (value) {
                    ...
                    if (value === true) {
                        ...
                        tools.modules.db.removeGame(gameID, function (err) {
                            if (err) {
        ...
        socket.on('watch', function onWatch() {
            socket.join('games');

            socket.emit('games', tools.data.getGames());

            tools.data.on('update', function () {

                io.sockets.in('games').emit('games', tools.data.getGames());
            });

        });

        socket.set('jeffTest', 'thisismytest text', function () {
            socket.get('jeffTest', function (err, text) {
                debug.emit('val', 'jeffTest', text);
            });
        });
