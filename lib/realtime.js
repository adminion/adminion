...
function realtime (tools) {
    ...
    function roster (players) {
        ...
        for ( i in players) {
            ...
            account = (player) ? tools.modules.cache.getAccount(player.account) : undefined;
    ...
    function initSocket (socket) {
           ...
           // assign event handler for when the player says "i'm ready!"
            socket.on('gameReady', function onGameReady (value) {
                ...
                if (startTimer && !value) {
                    ...
                    io.sockets.in('games/' + gameID).emit('gameStarting', false);
                }
                    ...
                    if (game.allReady()) {
                        ...
                        startTimer = setTimeout(function startTimerCallback () {
                            ...
                            game.startGame(function gameStarted (err, savedGame) {
                                ...
                                tools.modules.cache.setGame(savedGame);
                ...
                socket.on('gameKill', function onGameKill (value) {
                    ...
                    if (value === true) {
                        ...
                        tools.modules.db.removeGame(gameID, function (err) {
                            ...
                            tools.modules.cache.removeGame(gameID);
        ...
        socket.on('watch', function onWatch() {
            ...
            socket.emit('games', tools.modules.cache.getGames());

            tools.modules.cache.on('update', function () {

                io.sockets.in('games').emit('games', tools.modules.cache.getGames());
            });
