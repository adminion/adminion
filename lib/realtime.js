...
module.exports = function (tools) {
    ...
    function roster (players) {
        ...
        for ( var playerNo in players ) {
            ...
            var account = tools.Active.Accounts[accountID];
    ...
    var io = socketio.listen(tools.http.server);
    ...
    io.sockets.on('connection', function (socket) {
        debug.marker('socket connect', 'lib/realtime', 72);
        ...
        socket.on('joinGame', function () {
            ...
            var game = tools.Active.Games[gameID];

            // if the game is undefined
            if (!game) {
                
                debug.val('game', game, 'lib/realtime', 103);
            ...
            if ( openSeats > 0 ) {
                ...
                if ( game.isRegistered(accountID) === false) {
                    ...
                    debug.msg(util.format('%s entered game lobby %s', handle, gameID), 'lib/realtime.js', 137);
                }
        ...
        socket.on('disconnect', function () {
            debug.marker('socket disconnect', 'lib/realtime', 177);
