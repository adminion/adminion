...
var config = require('./config'),
    env = require('./env'),
    SocketStoreage = require('./models/socket'),
    utils = require('./utils');
...
function AdminionServer_realtime (modules) {
    ...
    this.start = function () {
        ...
        var configuration = {
            'authorization': passportSocketIo.authorize({
                cookieParser:   modules.http.cookieParser, 
                key:            'adminion.sid',
                secret:         config.session.secret, 
                store:          modules.data.session()
            }),
            'log level': (config.debug) ? 3 : 0
        };

        // create server instance
        io = socketio.listen(modules.http.server, configuration);
    ...
    function roster (players) {
        ...
        for ( i in players) {
            ...
            account = (player) ? modules.data.getAccount(player.account) : undefined;
    ...
    function initSocket (socket) {
        ...
        function onDisconnect () {
            ...
            socket.get('gameID', function (err, gameID) {
                if (!err) {
                    ...
                    game = modules.data.getGame(gameID);
            ...
            if (game.isPlayerOne(accountID)) {
                    ...
                    if (game.allReady()) {
                        ...
                        startTimer = setTimeout(function startTimerCallback () {
                            ...
                            game.startGame(function gameStarted (err, savedGame) {
                                ...
                                modules.data.setGame(savedGame);
                ...
                socket.on('gameKill', function onGameKill (value) {
                    ...
                    if (value === true) {
                        ...
                        modules.data.removeGame(gameID, function (err) {
                            if (err) {
                            ...
                            modules.data.removeGame(gameID);
        ...
        function onWatch() {
            ...
            socket.emit('games', modules.data.getGames());

            modules.data.on('update', function () {

                io.sockets.in('games').emit('games', modules.data.getGames());
            });
...
util.inherits(AdminionServer_realtime, events.EventEmitter);

module.exports = AdminionServer_realtime;
