    ...
    GameSchema.method({
        ...
        isRegistered: function (accountID) {
            ...
            for (var i = 0; i < this.registeredPlayers.length; i += 1) {
                ...
                debug.emit('val', 'player.accountID', player.accountID, 'models/game.js', 188); 

                debug.emit('val', 'new player', accountID, 'models/game.js', 190);
                debug.emit('val', 'registered player ' + i, player.accountID, 'models/game.js', 191);

                if (accountID.toString() === player.accountID.toString() ) {
                    debug.emit('msg', 'accountIDs match--player is registered.', 'models/game.js', 194);
                    return i;
                } else {
