...
module.exports = function (mongoose) {
    ...
    GameSchema.method({
        ...
        playersConnected: function () {
            ...
            debug.emit('val', 'connected', connected, 'models/game.js', 114);
        ...
        isPlayerOne: function (accountID) {
            ...
            debug.emit('msg', MSG_NOT_PLAYER_ONE, 'models/game.js', 218);
            return false;
        },
        ...
        isRegistered: function (accountID) {
            ...
            for (var i = 0; i < this.registeredPlayers.length; i += 1) {
                ...
                debug.emit('msg', 'index ' + i , 'models/game.js', 143);
                debug.emit('val', 'player.accountID', player.accountID, 'models/game.js', 144); 

                debug.emit('val', 'new player', accountID, 'models/game.js', 155);
                debug.emit('val', 'registered player ' + i, player.accountID, 'models/game.js', 156);

                if (accountID.toString() === player.accountID.toString() ) {
                    debug.emit('msg', 'accountIDs match!', 'models/game.js', 151);
                    return i;
                } else {
                    debug.emit('msg', 'accountIDs DO NOT match!', 'models/game.js', 154);
                }
            };
        ...
        register: function (accountID) {
            ...
            debug.emit('val', 'accountID', accountID, 'models/game.js', 169);
            debug.emit('val', 'this.registeredPlayers', this.registeredPlayers, 'models/game.js', 170);

            debug.emit('msg', accountID + ' is now registered.', 'models/game.js', 163);

            // define the new player
            this.registeredPlayers.addToSet({ accountID: accountID });

            debug.emit('val', 'this.registeredPlayers', this.registeredPlayers, 'models/game.js', 173);
        ...
        exitLobby: function (accountID) {
            ...
            debug.emit('val', 'index', index, 'models/game.js', 182);

            // if the index matches the return value of this.isRegistered()...
            if ( index !== false ) {
                this.registeredPlayers[index].remove();

                debug.emit('val', 'this.registeredPlayers', this.registeredPlayers, 'models/game.js', 188);
                ...
                debug.emit('msg', 'player not found', 'models/game.js', 195);
                return false;
            }
        }   
    });
