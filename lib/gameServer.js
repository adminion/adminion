...
//			debug.emit('var' , 'request.session', request.session, 'lib/gameServer.js', 140);
	...
	app.post('/games', ensureAuth, function appPostGames(request, response) {
		...
		debug.vars( 'request.user', request.user, 'lib/gamesServer.js', 457);
			...
			db.Game.findById(gameId, function(err, game) {
				...
				// if there is no error
					...
					if (!games[gameId]) {
						...
						games[gameId] = new Roster(gameId, game.playerOne, game.config.maxPlayers);
					...
					if (games[gameId].add(socket)) {
						...
						socket.set('gameId', gameId, function setGameId() {
							...
							// Notify all players of which player joined and the new list of players
							io.sockets.in(gameId).emit('joined', socket.id, games[gameId].getPlayers());

							debug.vars('games[' + gameId + '].getPlayers()',games[gameId].getPlayers(), 'lib/gameServer.js', 697);

						});
					} else {
						var err = 'something went wrong when adding the socket to the roster';
						console.trace(err)
						console.log(err);
						socket.disconnect();
		...
		socket.on('disconnect', function onDisconnect() {
			...
			socket.get('gameId', function(gameId) {
				...
				if (games[gameId]) {
					...
					debug.vars('games[' + gameId + '].getPlayers()',games[gameId].getPlayers(), 'lib/gameServer.js', 723);
				}
			});
		});
