...
//			debug.emit('var' , 'request.session', request.session, 'lib/gameServer.js', 140);
	...
	app.get('/games/:gameId/lobby', ensureAuth, function(request, response) {
		debug.vars( 'request', request.session, 'lib/gamesServer.js', 479);
...
function realtimeInit() {
	...
	debug.vars( 'express.cookieParser', express.cookieParser, 'lib/gameServer.js', 557);
	...
	io.sockets.on('connection', function(socket) {
		// console.log(player);
		debug.vars( 'socket.handshake', socket.handshake, 'lib/adminion', 568);

		// when the user wants to join a game
		socket.on('join', function(gameId) {
			debug.vars('gameId', gameId, 'lib/gameServer.js', 576);

			if (gameId === undefined) {
				debug.vars('gameId', gameId, 'lib/gameServer.js', 579);
				// handle error
				return;
			}
			...
			db.Game.findById(gameId, function(err, game) {
				...
				// if there is no error
					...
					debug.msg('game not found', 'lib/gameServer.js', 593);
					...
					if (!games[gameId]) {
						...
						games[gameId] = new Roster(game.playerOne, game.config.maxFriends);
					}

					// only inspect socket if there are open seats
					if (!!games[gameId].openSeats()) {						
						
						debug.msg('attempting to connet socket to game lobby...', 'lib/gameServer.js', 610);

						// if the socket is added successfully
						if (games[gameId].add(socket)) {
							// join the socket to the chat room "gameId"
							socket.join(gameId);
							sockets[socket.id] = { gameId : gameId };

							debug.vars('socket[socket.id]', sockets[socket.id], 'lib/gameServer.js', 620);

							socket.on('ready!', function(value) {
								games[gameId].ready(socket.id);
							});

							debug.vars('games[gameId].friends', games[gameId].friends, 'lib/gameServer.js', 620);
							console.log('%s joined game %s', socket.handshake.user.handle, gameId);
							
							// greet the new player 
							socket.emit('msg', "Welcome, " + socket.handshake.user.handle);

							// tell everyone in the room that the new player entered
							socket.broadcast.emit('joined', socket.handshake.user.handle, games[gameId].seats);
							//console.log(player);
						} 

					} else {
						socket.emit('deinied', 'no_empty_seats');
						socket.disconnect();
					} 
				}
			})
		...
		socket.on('disconnect', function(socket) {
			...
			debug.vars('sockets[socket.id]',sockets[socket.id], 'lib/gameServer', 647);
			// we'll make sure its only one id...
			var gameId = sockets[socket.id].gameId;

			// remove the socket from the lobby, if they're in there
			return games[gameId].remove(socket.id);
