...
function realtimeInit() {
	...
	io.sockets.on('connection', function onConnection(socket) {
		...
		socket.on('join', function onJoin(gameId) {
			debug.vars( 'socket.handshake', socket.handshake, 'lib/adminion', 629);
			var seatsRemaining;
			...
			db.Game.findById(gameId, function(err, game) {
				debug.vars( 'socket.handshake', socket.handshake, 'lib/adminion', 642);
				// if there's an error
				if (err) { 
				...
				// if there is no error
					...
					debug.vars( 'socket.handshake', socket.handshake, 'lib/adminion', 674);	

					seatsRemaining = games[gameId].add(socket);

					debug.vars( 'socket.handshake', socket.handshake, 'lib/adminion', 679);

					// if the socket is added successfully
					if (seatsRemaining) {
						debug.vars( 'socket.handshake', socket.handshake, 'lib/adminion', 683);
						var handle = socket.handshake.user.handle;
						
						socket.set('gameId', gameId, function() {
							console.log('%s joined game %s', handle, gameId);
