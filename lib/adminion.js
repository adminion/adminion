...
function Adminion() {
	...
	this.expressInit = function() {
		...
		this.app.get('/', function(request, response) {
			self.emit('debug' , 'request.session', request.session, 'lib/adminion.js', 142);
		...
		this.app.get('/players/:email', this.ensureAuth, function(request, response) {
			self.emit('debug' , 'request.params', request.params, 'lib/adminion.js', 242);
			self.db.Player.findOne({email: request.params.email}, function(err, player) {
				if (err) { self.emit('error', err); }
				self.emit('debug' , 'player', player, 'lib/adminion.js', 247);
		...
		this.app.post('/players/register', function(request, response) {
			self.emit('debug', 'this', this, 195); // when this is called, this points to Global
			self.emit('debug', 'self', self, 196); // self refers to the adminion module as it should
	...
	//////////////////////////////////////////////////////////////////////////// 
		...
		this.app.get('/games', this.ensureAuth, function(request, response) {
			response.render('games', {request : request});
		});
		...
		this.app.get('/games/:game/join', this.ensureAuth, function(request, response) {
			response.render('join', {request : 	request});
		});
	...
	this.ensureAuth = function (request, response, next) {
		...
		self.emit('debug' , 'request.session', request.session, 'lib/adminion.js', 249);
		if (request.isAuthenticated()) {
			...
			var url = util.format('/logon?redir=%s&err=%s',
				request.url,
	...
	this.startServer = function() {
		...
		if (this.config.https) {
			...
			this.server = https.createServer(this.config.https.data, this.app )
				.listen(this.config.port, function() { self.emit('serverReady'); });
		} else {
	...
	this.listening = function() {
		var secure = (this.config.https) ? ' for secure connections' : '';
		 
		console.log("Adminion game server '%s' listening%s.\nJoin game --> %s/join", 
			this.env.serverName, secure, this.env.url
		);
		this.emit('listening');
	};
};
...
gameServer.once('expressReady', gameServer.startServer);
gameServer.once('serverReady', gameServer.listening)

gameServer.emit('debug', 'gameServer', gameServer, 'lib/adminion.js', 303);

if(gameServer.config.debug) {
	gameServer.on('debug', debug.vars);
	gameServer.on('debugMsg',debug.msg);
}
